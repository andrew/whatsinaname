...
module Anagram
  ...
  def self.words
    @words ||= File.readlines("words").collect { |line| line.chomp.downcase }.select{|line| line.length < MAX_LENGTH }
  end
  ...
  def self.create_data
    File.open DataFilename, 'w' do |file|
      words_by_anagram_class.keys.sort.each do |ac|
        w = words_by_anagram_class[ac].join(' ')
        file.puts ac + ' ' + w if w.length < MAX_LENGTH
      end
    end
  end
  ...
  def self.two_word_anagrams_of(string, stoppage = 1)
